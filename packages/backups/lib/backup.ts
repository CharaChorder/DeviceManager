import type {
  ...
  CharaLayoutFile,
  CharaSettingsFile,
} from "apps/manager/src/lib/share/chara-file.js";
import type { Change } from "apps/manager/src/lib/undo-redo.js";
import {
  changes,
  ChangeType,
  chords,
  layout,
  settings,
} from "apps/manager/src/lib/undo-redo.js";
import { get } from "svelte/store";
import { serialPort } from "../../../apps/manager/src/lib/serial/connection";
import {
  csvLayoutToJson,
  isCsvLayout,
} from "packages/backups/lib/compat/legacy-layout";
...
export function downloadFile<T extends CharaFile<string>>(contents: T) {
  ...
  element.setAttribute(
    "download",
    `${contents.type}-${get(serialPort)
      ?.device}-${new Date().toISOString()}.json`,
  );
